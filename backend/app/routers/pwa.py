...
async def share_page(event_id: str, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    html = (
        ...
        "<script>"
        "const JOIN_URL='" + join_url + "';const RESULT_URL='" + result_url + "';"
        "const WA_RESULT='" + wa_result_url + "';"
        ...
        "function doShareResult(){if(navigator.share){navigator.share({title:'The plan is ready!',text:'The plan is ready! See it here',url:RESULT_URL}).catch(()=>{});}"
        "else{window.open(WA_RESULT,'_blank');}}"
        "</script>"
        "</div></body></html>"
    )
    return HTMLResponse(content=html)
